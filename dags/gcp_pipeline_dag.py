...
PROJECT_ID = Variable.get("GCP_PROJECT_ID", default_var="datapath-lab-airflow-485723")
SUBSCRIPTION_ID = "sales-sub"
# BUCKET_NAME = Variable.get("GCS_BUCKET_NAME", default_var="datapath-sales-bucket")
BUCKET_NAME = Variable.get("GCS_BUCKET_NAME", default_var="datapath-sales-bucket-datapath-lab-airflow-485723")
...
with DAG(
    ...
    def process_and_upload_to_gcs(ti):
        # pulled_messages = ti.xcom_pull(task_id='pull_pubsub_messages', key='return_value')
    ...
    load_to_bq = GCSToBigQueryOperator(
        ...
        bucket=BUCKET_NAME,
        # source_objects=["{{ task_instance.xcom_pull(task_id='process_and_upload_gcs') }}"],
    ...
    sql_agg = f"""
        ...
        SELECT
        product_id,
        DATE(SAFE_CAST(timestamp AS TIMESTAMP)) AS sale_date,
        COUNT(*) AS total_orders,
        SUM(quantity) AS total_quantity,
        SUM(total_amount) AS total_revenue
        FROM `{PROJECT_ID}.{DATASET_ID}.{TABLE_RAW}`
        WHERE SAFE_CAST(timestamp AS TIMESTAMP) IS NOT NULL
        GROUP BY 1, 2
